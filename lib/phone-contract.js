...
class PhoneContract extends Contract {
    ...
    async sellPhone(ctx, phoneId, carId, customerId) {
        ...
        let carrierData = this.readItem(ctx, carId);
        let carrier;
        if (carrierData) {
    ...
    //calculate sales volume for entity
    async displaySales(ctx, entityId) {
        let entity = await this.readItem(ctx, entityId);
        if (!entity) {
            throw new Error(`The entity ${entityId} does not exist`);
        }
        let deviceData = this.readItem(ctx, entity.devices[0]);
        let device = JSON.parse(JSON.stringify(deviceData)) ;
        return device;
        // for(let i = 0; i < entity.devices.length; i++){
        //     let device = this.readItem(ctx, entity.devices[i]);
        //     let price = parseFloat(device.price);
        //     return price;
        // }
